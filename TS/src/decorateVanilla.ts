...
import { decorate, param } from './helpers';
...
(async function () {
    const bookRepo: BookRepository = new (LogTimingDecorator(BookRepository))(books);

    decorate([TimingDecorator(true), param(0, ImportantDecorator)], bookRepo, "getBookById", { value: bookRepo.getBookById });
